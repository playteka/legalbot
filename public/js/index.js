...
$(document).ready(function() {
  ...
  var startConversation = function(index) {
    ...
    $.post("/question/checkout", {"question": data.response[1]})
    .done(function(result) {
        //alert("数据：" + JSON.stringify(result.body) + "\n状态：" + result.status + result.errorno);

        if (result.status == "error") {
            if (result.errorno == 1) { //not exist.
                alert('the next question does not exist');
            } 
        } else { //result.status == "success"
            var record = result.body;
            var text = he.escape(record.text);
            $('<p class="chat-watson"/>')
...
/*      //original code of IBM
      var text = data.response.join('&lt;br/&gt;');
      $('<p class="chat-watson"/>')
        .html($('<div/>').html(text).text())
        .appendTo($conversation);
*/
    }); // post v1/dialogs/dialog_id/conversation done
  }; //startConversation
  ...
  var conductConversation = function(){
    ...
    $.post(path, params).done(function(data) {
        console.log(data.response);
        //checkout the question record from db
        $.post("/question/checkout", {"question": data.response[0]})
        .done(function(result) {
            //alert("数据：" + JSON.stringify(result.body) + "\n状态：" + result.status + result.errorno);
      ...
      /* //original code of IBM
      var text = data.response.join('&lt;br/&gt;');
      var newtext = "<br>new line <br>new line again";
     
     newtext =  he.escape(newtext);
     text = text + newtext;
